/**
 * TripSummaryCard.jsx — Route card with dual-tone left pill,
...
export default function TripSummaryCard({
  ...
  const score =
    typeof biyahe_score === "object"
      ? biyahe_score?.biyahe_score
      : biyahe_score;
  ...
  const cardShadow = isRecommended
    ? ""
    : "shadow-[0px_4px_6px_rgba(0,0,0,0.05)]";
  ...
  const visibleSegments = segments.filter(
    (seg) => seg.distance_m > 0 || !seg.is_transfer
  );
...
- Other`);
    ...
    if (reason) {
      window.alert(
        "Thank you! Your report has been submitted. We'll review this route."
      );

      // TODO: POST to /api/v1/route-reports
    }
  };

  return (
    <div
      className={`${cardBg} ${cardShadow} rounded-2xl overflow-hidden border border-gray-100`}
    >
...
/**
 * RouteCardList — Renders primary + alternative routes in a stack.
 */
