    ...
    <nav className="sticky top-0 z-[120] border-b border-black/5 bg-white/90 px-4 py-1.5 backdrop-blur-xl shadow-[0_8px_30px_rgba(15,23,42,0.06)] sm:px-8 lg:px-10">
      <div className="mx-auto flex h-[3.8rem] w-full max-w-[1680px] items-center justify-between gap-4 lg:grid lg:grid-cols-[1fr_auto_1fr]">
        ...
        <div className="hidden lg:flex items-center justify-center px-2 xl:px-4 lg:justify-self-center">
          <div className="relative grid h-[2.8rem] w-[32rem] grid-cols-4 items-center rounded-full border border-black/5 bg-gray-100/90 p-1 shadow-[0_10px_30px_rgba(15,23,42,0.05)] ring-1 ring-white/70">
            <span
              aria-hidden
              className="pointer-events-none absolute left-1.5 top-1.5 h-[calc(100%-0.75rem)] w-[calc((100%-0.75rem)/4)] rounded-full bg-white shadow-[0_8px_18px_rgba(79,0,205,0.12)] transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]"
            ...
            {NAV_LINKS.map(({ to, label }) => {
              ...
              return (
                <Link
                  key={to}
                  to={to}
                  className={`relative z-[1] flex h-full items-center justify-center rounded-full px-5 text-[1.02rem] font-semibold leading-none tracking-[0.01em] transition-colors duration-200 whitespace-nowrap ${
                    isActive
                      ? 'text-[#4f00cd]'
      ...
      {mobileOpen && (
        <>
          ...
          <div
            className="md:hidden fixed inset-0 z-[130] bg-black/40"
            aria-hidden={!mobileOpen}
            onClick={closeMobile}
          />

          {/* Mobile: full-screen light menu panel */}
          <div
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
          ...
          <button
            type="button"
            className="p-2 rounded-xl text-gray-600 hover:bg-gray-100 -mr-1"
        ...
        <nav className="flex flex-col px-6 pt-6 pb-6 gap-2 flex-1 overflow-y-auto">
          ...
          <Link
            to="/signup"
            onClick={closeMobile}
            className="mt-3 inline-flex min-h-11 items-center justify-center rounded-full bg-[#4f00cd] px-5 py-3 text-base font-bold text-white shadow-[0_12px_24px_rgba(79,0,205,0.2)]"
          >
            Sign Up
          </Link>
        </nav>
