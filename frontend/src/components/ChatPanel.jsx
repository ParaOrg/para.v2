...
export default function ChatPanel() {
  ...
  useEffect(() => {
    ...
    const drawBg = (routes) => {
      ...
      const coords = [];
      routes.forEach(r => (r.coords || []).forEach(([f, t]) => {
        if (f && t && f.length >= 2 && t.length >= 2) {
          const ll = [[f[1], f[0]], [t[1], t[0]]];
          L.polyline(ll, { color: "#fbcfe8", weight: 1.5, opacity: 0.5 }).addTo(bg);
          coords.push(ll[0], ll[1]);
        }
      }));
      if (coords.length) try { map.fitBounds(L.latLngBounds(coords), { padding: [30, 30] }); } catch {}
    ...
    fetch(`${API_BASE}/admin/routes/verified`).then(r => r.json()).then(d => {
