    ...
    <>
      ...
      {showComparison && (
        <div className="mx-4 mt-4 mb-4 border-2 border-purple-300 rounded-xl bg-purple-50 p-4">
          ...
          {/* Safety Row -- only shown when the backend actually provides a rating */}
          {routeOptions.some((route) => route.safety_rating != null) && (
            <div className="grid grid-cols-4 gap-2 items-center">
              ...
              {routeOptions.slice(0, 3).map((route) => (
                <div key={route.route_id} className="text-center">
                  <p className="text-xs text-gray-700">{route.safety_rating != null ? `${route.safety_rating}/5` : "—"}</p>
                </div>
              ))}
            </div>
          )}
