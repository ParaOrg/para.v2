...
export default function RoutesExplorer() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API_BASE}/api/v1/jeepney-routes/manifest`).then(r => r.json()).catch(() => ({ verified: [] })),
      fetch(`${API_BASE}/api/v1/jeepney-routes/all`).then(r => r.json()).catch(() => ({ routes: [] })),
    ]).then(([manifest, aData]) => {
      setVerifiedRoutes(manifest.verified || []);
  ...
  // ── Draw Route ────────────────────────────────────────────
  const drawRoute = useCallback(async (routeKey) => {
    try {
      const res = await fetch(`${API_BASE}/api/v1/jeepney-routes/${encodeURIComponent(routeKey)}/geometry`);
