"""Build graph once and save to JSON file for Lambda to load"""
import json
import math
import os
...
def build():
    ...
    for shape in shapes:
        ...
        for i, coord in enumerate(sampled):
            node_id = f"{route['route_uuid']}_p{i}"
            if node_id not in nodes:
                nodes[node_id] = [coord[1], coord[0]]
                all_nodes.append([node_id, coord[1], coord[0]])
            if node_id not in graph:
                graph[node_id] = []
            if i > 0:
                prev_id = f"{route['route_uuid']}_p{i-1}"
    ...
    # Transfer edges - sorted by lat
    all_nodes.sort(key=lambda n: n[1])
    transfer_count = 0
    for i in range(len(all_nodes)):
        for j in range(i+1, len(all_nodes)):
            if all_nodes[j][1] - all_nodes[i][1] > 0.01:
                break
            dist = haversine(all_nodes[i][1], all_nodes[i][2], all_nodes[j][1], all_nodes[j][2])
            if dist < 0.5:
                walk_min = max(1, round(dist * 12))
                graph[all_nodes[i][0]].append([all_nodes[j][0], walk_min, "Transfer", "walk"])
                graph[all_nodes[j][0]].append([all_nodes[i][0], walk_min, "Transfer", "walk"])
                transfer_count += 1
    ...
    print(f"Graph built: {len(nodes)} nodes, {len(routes)} routes, {transfer_count} transfers")
