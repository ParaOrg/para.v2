...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(f"DEBUG Event: {json.dumps(event)[:500]}")
        
        # API Gateway wraps payload in 'body' as string
        if 'body' in event and isinstance(event['body'], str):
            try:
                event = json.loads(event['body'])
                print(f"DEBUG Parsed body: {json.dumps(event)[:500]}")
            except Exception as e:
        ...
        # Find nearest nodes (rail AND jeep)
        start_rail, start_rail_dist = find_nearest_node(user_lat, user_lng, 'rail::', 5.0)
        end_rail, end_rail_dist = find_nearest_node(dest_coords[0], dest_coords[1], 'rail::', 5.0)
        start_jeep, start_jeep_dist = find_nearest_node(user_lat, user_lng, 'jeep::', 3.0)
        end_jeep, end_jeep_dist = find_nearest_node(dest_coords[0], dest_coords[1], 'jeep::', 3.0)
        
        # Try all combinations and pick the best path
        candidates = []
        
        # Rail path (if start != end)
        if start_rail and end_rail and start_rail != end_rail:
            rail_path = astar_search(start_rail, end_rail, max_time_min=200)
            if rail_path:
                candidates.append(('rail', rail_path, start_rail_dist + end_rail_dist))
        
        # Jeep path (if start != end)
        if start_jeep and end_jeep and start_jeep != end_jeep:
            jeep_path = astar_search(start_jeep, end_jeep, max_time_min=200)
            if jeep_path:
                candidates.append(('jeep', jeep_path, start_jeep_dist + end_jeep_dist))
        
        # Mixed path: start rail → end jeep (transfer)
        if start_rail and end_jeep:
            mixed_path = astar_search(start_rail, end_jeep, max_time_min=200)
            if mixed_path:
                candidates.append(('mixed', mixed_path, start_rail_dist + end_jeep_dist))
        
        # Mixed path: start jeep → end rail (transfer)
        if start_jeep and end_rail:
            mixed_path2 = astar_search(start_jeep, end_rail, max_time_min=200)
            if mixed_path2:
                candidates.append(('mixed2', mixed_path2, start_jeep_dist + end_rail_dist))
        
        # If user is very close to destination (within 1km), just walk
        direct_walk_dist = haversine(user_lat, user_lng, dest_coords[0], dest_coords[1])
        if direct_walk_dist < 1000:  # 1km
            # Create a direct walking path
            walk_path = [('walk', user_lat, user_lng, 0, dest_coords[0], dest_coords[1], direct_walk_dist / 60 * 15)]  # 15 min/km walk speed
            candidates.append(('walk', walk_path, 0))
        ...
        # Pick the best candidate (shortest total time including walking to station)
        best_candidate = min(candidates, key=lambda c: len(c[1]))
        path_type, path, walk_dist = best_candidate
        
        print(f"DEBUG: Using {path_type} path with {len(path)} nodes")
