...
export default function HomeNew() {
  ...
  return (
    <div className="fixed inset-0 bg-white">
      <GpsPrompt />
      <div className="hidden md:block"><Navbar /><ChatPanel /><button onClick={locateMap} className="fixed top-20 right-4 z-[9999] bg-white w-10 h-10 rounded-full shadow-lg flex items-center justify-center hover:bg-gray-50 border border-gray-200"><GpsIcon size={20} /></button></div>
      <div className="md:hidden">
        ...
        <button onClick={() => window.dispatchEvent(new Event("para-show-weather"))} className="absolute top-4 right-16 z-[9999] bg-white w-10 h-10 rounded-full shadow-lg flex items-center justify-center hover:bg-gray-50 border border-gray-200"><span className="text-lg">🌤️</span></button>
        <button onClick={locateMap} className="absolute top-4 right-4 z-[9999] bg-white w-10 h-10 rounded-full shadow-lg flex items-center justify-center hover:bg-gray-50 border border-gray-200"><GpsIcon size={20} /></button>
        {chatOpen && !showTracker && (
          <div className="absolute left-2 right-2 z-20 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden" style={{ bottom: `calc(70px + ${kbOffset}px)`, maxHeight: showChat ? "50vh" : "auto", transition: "bottom 120ms ease-out" }}>
            ...
            <div className="flex items-center gap-2 px-3 py-2.5"><input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder={placeholder}
                title="Type like: from Cubao to Makati, or just: Makati. 🔒 Location only with consent. Privacy: https://www.para-commute.org/privacy-policy"
                enterKeyHint="search"
                autoComplete="off"
