...
export default function CommuteTracker({ routeData, onComplete, onCancel, onMinimize, onProgress }) {
  ...
  useEffect(() => {
    ...
    return () => { if (typeof stopTracking === "function") stopTracking(); };
  ...
  const finish = async () => {
    ...
    try { await edgePost("commute-save", log); } catch {
      ...
      queue.push(log);
      writeQueue(queue);
      setSaveMessage("Saved offline. It will sync automatically when connection returns.");
    }
    setSaving(false);
    if (onComplete) onComplete(log);
  };

  if (!segments.length) return <div className="bg-white rounded-xl p-6 text-center text-sm text-gray-500">No route segments available to track.</div>;
  ...
  if (minimized) {
    return (
      <div className="bg-[#7A4BC8] text-white px-4 py-2 flex items-center justify-between cursor-pointer rounded-xl" onClick={() => { setMinimized(false); if (onMinimize) onMinimize(); }}>
        <span className="text-xs font-bold">🚀 Tracking — {formatTime(totalTimeSec)}</span>
  ...
  if (!consent) {
    return (
      <div className="bg-white rounded-xl p-6 text-center space-y-4">
        ...
        <p className="text-sm text-gray-500">Tracking your commute helps improve route data and safety. Location collection starts only after you allow it.</p>
        {consentError && <div className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg p-2">{consentError}</div>}
        <button onClick={requestConsentAndLocation} className="w-full py-3 bg-[#7A4BC8] text-white rounded-xl font-bold text-sm hover:bg-[#6a3cb8] transition-colors">Enable tracking</button>
        <button onClick={onCancel} className="w-full py-2 border border-gray-300 text-gray-500 rounded-lg text-xs hover:bg-gray-50">Cancel</button>
        <p className="text-[11px] text-gray-400"><Link to="/privacy-policy" className="underline">Read the privacy policy</Link></p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl overflow-hidden shadow-lg flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 py-2 bg-gray-50 border-b border-gray-100 flex items-center gap-2 text-xs">
          ...
          <span className="text-gray-500">{consentError ? consentError : status === "watching" ? `GPS active (${gpsPoints.length} points)` : "GPS starting…"}</span>
        </div>
        <div className="px-4 py-2 flex gap-1">
          {segments.map((seg, i) => (
            <div key={i} className={`flex-1 h-1.5 rounded-full ${i < currentSegment ? "bg-green-500" : i === currentSegment ? "bg-purple-500 animate-pulse" : "bg-gray-200"}`} />
          ))}
        </div>
        <div className="p-4 space-y-3">
          ...
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Optional commute feedback..." rows={3} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-purple-500 resize-none" />
              ...
              <div className="text-xs space-y-1 bg-gray-50 rounded-lg p-2">
                ...
                <div className="flex items-center gap-2 text-gray-400 pt-1 border-t border-gray-200"><span>⏳ Wait</span><span className="ml-auto tabular-nums">{formatTime(waitTime)}</span></div>
              </div>
              {saveMessage && <div className="text-xs bg-blue-50 border border-blue-200 text-blue-700 rounded-lg p-2">{saveMessage}</div>}
              <button onClick={finish} disabled={saving} className="w-full py-3 bg-purple-800 text-white rounded-xl font-bold text-sm hover:bg-purple-700 transition-colors disabled:opacity-50">{saving ? "Saving..." : "💾 Save & Finish"}</button>
              <button onClick={() => { finish(); onCancel(); }} disabled={saving} className="w-full py-3 border-2 border-purple-800 text-purple-800 rounded-xl font-bold text-sm hover:bg-purple-50 transition-colors disabled:opacity-50">🚐 Commute Again</button>
              <button onClick={onCancel} className="w-full py-2 border border-gray-300 text-gray-500 rounded-lg text-xs hover:bg-gray-50">Discard</button>
            </div>
          )}
        </div>
      </div>
